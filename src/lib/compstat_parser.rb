...
class CompstatParser
  ...
  def process(pdf_data, pdf_path, trash=nil)
    ...
    if (@config['aws'] && @config['aws']['s3']) || ["AWS_ACCESS_KEY_ID", "AWS_SECRET_ACCESS_KEY", "AWS_S3_BUCKET_PATH", "AWS_S3_BUCKET"].all?{|key| ENV.has_key?(key) }
      ...
      S3Publisher.publish(ENV["AWS_S3_BUCKET"] || @config['aws']['s3']['bucket'], {logger: 'faux /dev/null'}){ |p| p.push(key, data: pdf_data, gzip: false) } if (ENV["AWS_S3_BUCKET"] || @config['aws']) && !@s3.buckets[(ENV["AWS_S3_BUCKET"] || @config['aws']['s3']['bucket'])].objects[key].exists?
    end
